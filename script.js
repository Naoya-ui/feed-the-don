(() => {
  ...
  function setBackground(name) {
    ...
    els.background.style.backgroundColor =
      name === "black" ? "#000" : "#08090a";
  }
  ...
  function showNameInput() {
    ...
    els.nameInput.value =
      state.playerName === "Manager" ? "" : state.playerName;
  ...
  function selectLocation(loc) {
    ...
    document.dispatchEvent(
      new CustomEvent("locationSelected", { detail: loc }),
    );
  }
  ...
  async function runLocation(loc) {
    ...
    if (loc === "Forest") {
      ...
      await say(
        "",
        "The original Ren'Py project does not contain the forest_main story yet. This is the end of the current demo content.",
      );
      await mapLoop();
    }
  }
  ...
  function resetUIForNewGame() {
    ...
    clearTimeout(backpackReminderTimer);
    closeChoices();
    [els.nameModal, els.inventoryModal, els.mapModal, els.systemModal].forEach(
      (el) => (el.hidden = true),
    );
    els.dialogueBox.hidden = true;
    els.character.hidden = true;
