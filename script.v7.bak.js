(() => {
  ...
  function createTurnSlots() {
    ...
    const intents = battle.enemySlots;
    battle.donSlots.forEach((slot, i) => {
      slot.targetIntentId = intents.length ? intents[i % intents.length].id : null;
    });
  ...
  function clashForecast(slot) {
    ...
    if (!enemySlot) return { label: "UNOPPOSED", cls: "unopposed", delta: 99 };
  ...
  function renderEnemyIntents() {
    ...
    battle.enemySlots.forEach((slot) => {
      ...
      btn.addEventListener("click", () => {
        ...
        const focus = battle.donSlots[battle.focusSlot];
        if (!focus) return;
        focus.targetIntentId = slot.id;
        renderPlanning();
      });
      btn.addEventListener("pointerup", () => {
        if (battle.phase !== "planning" || battle.dragSlot === null) return;
        const drag = battle.donSlots[battle.dragSlot];
        if (drag) drag.targetIntentId = slot.id;
  ...
  function beginTargetDrag(event, index) {
    if (battle.phase !== "planning") return;
    battle.dragSlot = index;
    battle.focusSlot = index;
    const slot = battle.donSlots[index];
    if (slot) showSkillInspector(getSelectedSkill(slot), slot);
    document.body.classList.add("lc4-targeting");
    const onMove = (moveEvent) => drawTargetLines({ x: moveEvent.clientX, y: moveEvent.clientY });
    const onUp = (upEvent) => {
      const target = document.elementFromPoint(upEvent.clientX, upEvent.clientY)?.closest?.(".lc4-intent");
      if (target && slot) slot.targetIntentId = target.dataset.intentId;
      battle.dragSlot = null;
  ...
  function tutorialForTurn() {
    const step = battleTutorialSteps[Math.min(battle.tutorialStep, battleTutorialSteps.length - 1)];
  ...
  async function resolvePlayerAction(slot) {
    ...
    let enemySlot = enemySlotById(slot.targetIntentId);
    if (!enemySlot || enemySlot.consumed || enemyById(enemySlot.enemyId)?.hp <= 0) {
      enemySlot = battle.enemySlots.find((intent) => !intent.consumed && enemyById(intent.enemyId)?.hp > 0) || null;
    }
    let enemy = enemySlot ? enemyById(enemySlot.enemyId) : aliveEnemies()[0];
    if (!enemy) return;
    const skill = getSelectedSkill(slot);

    if (!enemySlot || enemy.staggered) {
