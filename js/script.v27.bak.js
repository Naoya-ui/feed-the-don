(() => {
  ...
  function renderEnemyIntents() {
    ...
    battle.enemySlots.forEach((slot) => {
      ...
      btn.style.left = `${31 + ((enemyIndex + 0.5) * 67 / Math.max(1, living.length)) + sibIndex * 2.7}%`;
      btn.style.top = `${18 + (sibIndex * 2.2)}%`;
    ...
    battle.donSlots.forEach((slot) => {
      ...
      wrap.innerHTML = `
        ...
        <button class="lc4-slot-core" type="button" aria-label="Drag to target an enemy action">
          ...
          <span class="lc4-mini-portrait"><img src="assets/battle/icons/portrait/don-quixote.png" alt="" /></span>
